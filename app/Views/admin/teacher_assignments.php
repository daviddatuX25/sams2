...
<div class="container">
            ...
            <table id="assignmentsTable" class="table table-striped data-table">
                ...
                <tbody>
                    <?php foreach ($assignments as $assignment): ?>
                        <tr>
                            <td><?= esc($assignment['assignment_id']) ?></td>
                            ...
                            <td>
                                <button class="btn btn-sm btn-primary edit-btn" data-id="<?= $assignment['assignment_id'] ?>" data-url="<?= site_url('admin/teacher-assignments') ?>" data-bs-toggle="modal" data-bs-target="#editAssignmentModal">Edit</button>
                                <button class="btn btn-sm btn-danger delete-btn" data-id="<?= $assignment['assignment_id'] ?>" data-bs-toggle="modal" data-bs-target="#deleteAssignmentModal">Delete</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addAssignmentModalLabel">Add Teacher Assignment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="addAssignmentForm" class="ajax-form" method="post" action="<?= site_url('admin/teacher-assignments') ?>">
                        <input type="hidden" name="action" value="create">
                        <div class="mb-3">
                            <label for="addUserId" class="form-label">Teacher</label>
                            <select class="form-control" id="addUserId" name="user_id">
                                <?php foreach ($teachers as $teacher): ?>
                                    <option value="<?= $teacher['user_id'] ?>"><?= esc($teacher['first_name'] . ' ' . $teacher['last_name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="mb-3">
    ...
    <div class="modal fade" id="editAssignmentModal" tabindex="-1" aria-labelledby="editAssignmentModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editAssignmentModalLabel">Edit Teacher Assignment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editAssignmentForm" class="ajax-form" method="post" action="<?= site_url('admin/teacher-assignments') ?>">
                        <input type="hidden" name="action" value="update">
                        <input type="hidden" name="assignment_id" id="edit-assignment_id">
                        <div class="mb-3">
                            <label for="edit-user_id" class="form-label">Teacher</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="deleteAssignmentForm" class="ajax-form" method="post" action="<?= site_url('admin/teacher-assignments') ?>">
